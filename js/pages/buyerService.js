...
export const BuyerService = {
  ...
  startNegotiationForRfq: async (rfqId) => {
      ...
      if(rfq) {
          ...
          // دریافت آیدی چت برای هدایت کاربر
          const conv = await MessageService.createConversation([rfq.buyerId, rfq.supplierId], `مذاکره: تأمین ${product ? product.name : 'کالا'}`, dealId);
          convId = conv.id;
  ...
  submitTender: async (data) => {
    ...
    const newTender = {
      ...
      radarActivated: data.radarActivated || false 
    };
  ...
  acceptTenderProposal: async (tenderId, proposalId) => {
      ...
      if(tender) {
          ...
          const isFirstNegotiation = !tender.radarActivated;
          tender.radarActivated = true; 
          ...
          AppStore.addDeal({
              ...
              status: 'negotiating',
              pitches: [],
              showInRadar: isFirstNegotiation 
          });

          if(proposal) {
              // دریافت آیدی چت برای هدایت کاربر
              const conv = await MessageService.createConversation([tender.buyerId, proposal.supplierId], `مذاکره مناقصه: ${tender.title}`, dealId);
              convId = conv.id;
              AppStore.addNotification(proposal.supplierId, `پیشنهاد شما پذیرفته شد! خریدار مایل به مذاکره است.`, 'success');
          }
