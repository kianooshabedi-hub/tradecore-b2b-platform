// js/data/businesses.js

export const businesses = [
  {
    id: "biz-1", // POUYA-NEGAR-001
    name: "شرکت صنایع بسته‌بندی پویا نگار",
    englishName: "Pouya Negar Packaging Industries",
    logo: "images/businesses/pouya-negar-logo.jpg",
    country: "ایران",
    city: "اصفهان",
    industry: "تولید ظروف فلزی و روغن‌های صنعتی",
    description: "اولین مجتمع تولید ظروف فلزی صنعتی و غذایی در استان اصفهان. مجهز به خطوط کامل لیتوگرافی، چاپ، ساخت اتوماتیک قوطی و تولید روغن موتور با برند تورال (Toral).",
    website: "اطلاعات در دسترس نیست",
    email: "info@pouyanegar.local", // دیتای پیش‌فرض برای دمو
    phone: "+98-31-0000000",
    status: "verified",
    joinDate: "1383", // بر اساس سال تاسیس
    certifications: ["ISO 9001:2000", "ISO 14001:2004", "OHSAS 18001", "ISO 17025"]
  },
  {
    id: "biz-2", // EVRIN-001
    name: "Petro-Payesh Evrin Ravan Co.",
    englishName: "Petro-Payesh Evrin Ravan Co.",
    logo: "images/businesses/evrin-logo.jpg",
    country: "ایران",
    city: "تهران",
    industry: "تولید روانکارهای تخصصی (Lubricants)",
    description: "تولیدکننده تخصصی انواع روانکارهای دریایی (Marine)، صنعتی، خودرویی و سیالات انتقال قدرت با نام تجاری PAGO. (شعار: FULL STEAM AHEAD!)",
    website: "www.evrinoil.com",
    email: "contact@evrinoil.com",
    phone: "+98-21-0000000",
    status: "verified",
    joinDate: "1390",
    certifications: ["ISO 9001", "API Standards"]
  },
  {
    id: "biz-3", // ARIAN-PETRO-IDEA-001
    name: "آرین پترو ایده (API)",
    englishName: "Arian Petro Idea",
    logo: "images/businesses/api-logo.jpg",
    country: "ایران",
    city: "تهران",
    industry: "تولید روانکارها (دانش‌بنیان)",
    description: "شرکت دانش‌بنیان تولیدکننده انواع روغن‌های موتوری، صنعتی، گریس و تجهیزات مرتبط با خطوط لوله. (شعار: Where Quality meets Creativity)",
    website: "www.api.co.ir",
    email: "info@api.co.ir",
    phone: "+98-21-1111111",
    status: "verified",
    joinDate: "1386",
    certifications: ["Knowledge-Based Company"]
  }
];